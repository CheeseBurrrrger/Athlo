...
class UpdateGoalVariablesBuilder {
  ...
  Deserializer<UpdateGoalData> dataDeserializer = (dynamic json) =>
      UpdateGoalData.fromJson(jsonDecode(json));
...
class UpdateGoalGoalUpdate {
  ...
  const UpdateGoalGoalUpdate({required this.id});
}
...
class UpdateGoalData {
  ...
  UpdateGoalData.fromJson(dynamic json)
    : goal_update = json['goal_update'] == null
          ? null
          : UpdateGoalGoalUpdate.fromJson(json['goal_update']);
  @override
  bool operator ==(Object other) {
    if (identical(this, other)) {
      return true;
  ...
  const UpdateGoalData({this.goal_update});
}
